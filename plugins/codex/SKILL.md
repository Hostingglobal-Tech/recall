...
`recall` 은 Codex 의 `~/.codex/sessions/**/rollout-*.jsonl` (모던) + `~/.codex/history.jsonl` (레거시 fallback) 양쪽 모두 인덱싱한다. `~/.codex/session_index.jsonl` 에서 `thread_name` 을 가져와 세션 title 로 매핑한다.
...
Claude Code 도 함께 쓰는 머신이라면 동일한 단계로 `~/.claude/skills/recall/` 에도 `plugins/claude/SKILL.md` 를 복사해두면 양쪽 CLI 모두에서 자동 호출된다.
